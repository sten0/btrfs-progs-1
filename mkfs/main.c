...
/*
 * Just update chunk allocation info, since --rootdir may allocate new chunks
 * which are not updated in @allocation structure.
 */
				    ...
				    struct mkfs_allocation *allocation)
{
	...
	allocation->system = 0;
	while (1) {
		bg_cache = btrfs_lookup_first_block_group(fs_info, search_start);
		if (!bg_cache)
			break;
